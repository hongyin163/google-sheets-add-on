 ...
 */
 ...
 import { REQUIREMENT_COLUMN_CONFIG } from '../../../_share/config/field-config'
...
const getTaskKeysFromRangeListValues = (rangeListValues: RangeListValues): string[] => {
  const epicIndex = getFieldIndex('Epic', REQUIREMENT_COLUMN_CONFIG)
  const keyIndex = getFieldIndex('Task', REQUIREMENT_COLUMN_CONFIG)
  ...
  rangeListValues.forEach(rangeValue => {
    rangeValue?.forEach(row => {
      if (row && row[epicIndex] && (!row[keyIndex] || !/\d+$/.test(`${row[keyIndex]}`))) {
        let key = `${row[epicIndex]}`
        key = key.trim(); // 去除头尾空格
        if (numReg.test(key)) {
          key = `SPML-${key}`
        }
 ...
 const updateRangeListValuesFromIssues = (rangeListValues: RangeListValues, issues: Issue[], fieldKey?: string): RangeListValues => {
  const issuesHash: {[key: string]: Issue} = {}
  issues.forEach((issue) => {
    // issueType === 'Epic'
    if (issue.fields.issuetype.id === '10004') {
      issuesHash[issue.key] = issue
    }
  })
  const keyIndex = getFieldIndex('Epic', REQUIREMENT_COLUMN_CONFIG)
  const numReg = /^\d+$/
   ...
   return new Promise((resolve, reject) => {
     google.script.run
       ...
       .withFailureHandler((error, object) => {
         console.log('Error in `completeEpic`.', error, object)
         reject(error)
       })
       .completeTask({
         rows,
         values,
         keyIndex: getFieldIndex('Epic', REQUIREMENT_COLUMN_CONFIG)
       })
   })
 }
 ...
 export default withJiraStatusCheck(Processing,'complete-epic')
